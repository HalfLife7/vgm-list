...
<div>
    <div class="flex flex-wrap container m-auto pt-20">
        <div class="w-full md:w-2/6">
            <img v-if="gameDataIsLoaded" class="w-full" :src="game.covers[0].url" alt="Game Cover" />
        </div>
        <div v-if="gameDataIsLoaded" class="w-full p-4 md:w-4/6 md:p-0 md:pl-4">
            ...
            <div class="flex">
                <div v-for="(website, index) in game.websites" :key="index" :website="website">
                    <div class="mr-5 mt-5">
    ...
    <div class="flex container m-auto pt-20">
        <ul class="flex border-b">
            ...
            image === 'screenshot' ? 'is-active bg-gray-200 text-blue-700' : '',
                ...
                <a class="inline-block border-l border-t border-r rounded-t py-2 px-4 font-semibold hover:text-blue-700 cursor-pointer" @click="image = 'screenshot'">Screenshots</a>
            </li>
            <li class="-mb-px mr-1" :class="[
            image === 'artwork' ? 'is-active bg-gray-200 text-blue-700' : '',
          ]">
                <a class="inline-block border-l border-t border-r rounded-t py-2 px-4 font-semibold hover:text-blue-700 cursor-pointer" @click="image = 'artwork'">Artwork</a>
            </li>
            <li class="-mb-px mr-1" :class="[
            image === 'video' ? 'is-active bg-gray-200 text-blue-700' : '',
          ]">
                <a class="inline-block border-l border-t border-r rounded-t py-2 px-4 font-semibold hover:text-blue-700 cursor-pointer" @click="image = 'video'">Videos</a>
            </li>
        </ul>
    </div>
    <div class="image-content">
        <div v-show="image === 'screenshot'" class="flex container m-auto">
            <swiper v-if="gameDataIsLoaded" class="swiper" :options="swiperOption">
                <swiper-slide v-for="(screenshot, index) in game.screenshots" :key="index" :screenshot="screenshot">
        ...
        <div v-show="image === 'artwork'" class="flex container m-auto">
            <swiper v-if="gameDataIsLoaded" class="swiper" :options="swiperOption">
                <swiper-slide v-for="(artwork, index) in game.artworks" :key="index" :artwork="artwork">
        ...
        <div v-show="image === 'video'" class="flex container m-auto">
            <swiper v-if="gameDataIsLoaded" class="swiper" :options="swiperOption">
                <swiper-slide v-for="(video, index) in game.videos" :key="index" :video="video">
    ...
    <div v-for="(album, index) in albums" :key="index" :album="album" class="pt-20">
        <div class="flex flex-wrap container m-auto">
            <div class="w-full md:w-2/6">
                <img v-if="albumDataIsLoaded" :src="album.data.covers[0].full" :alt="album.data.name + ' Album Cover'" />
            </div>
            <div v-if="albumDataIsLoaded" class="w-full p-4 md:w-4/6 md:p-0 md:pl-4">
                <div class="text-4xl">{{ album.data.name }}</div>
                <div class="text-xl">Release Date: {{ album.data.release_date }}</div>

                <div class="flex">
                    <div v-for="(store, index2) in album.data.stores" :key="index2" :store="store">
                        <div class="mr-5 mt-5">
                            <a :href="store.link" target="_blank" rel="noopener" class="flex">
        ...
        <div class="flex flex-wrap container m-auto justify-center mt-4">
            ...
            <vue-good-table class="w-full lg:w-1/2 mt-4" :columns="columns" :rows="album.data.tracks" :search-options="{
            enabled: true,
            trigger: 'enter',
            skipDiacritics: true,
